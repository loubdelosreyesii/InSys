...
{
    internal class RaffleEntryRepository : IRaffleEntry
    {
        private RaffleEntry _record;

        public RaffleEntry Record
        {
            get => _record;
            set => _record = value;
        }
        ...
        public List<RaffleEntry> SelectAll()
        {
            ...
            {
                var record = db.RaffleEntries.ToList();
        ...
        public List<RaffleEntry> SelectAll(int paramRaffleId)
        {
            ...
            using (var db = new X2MO_InSysEntities())
            {
                var record = db.RaffleEntries.Where(p=>p.RaffleId ==paramRaffleId).ToList();
        ...
        public RaffleEntry Select(string paramRaffleNumber)
        {
            ...
            using (var db = new X2MO_InSysEntities())
            {
                var record = db.RaffleEntries.SingleOrDefault(p => p.RaffleReferenceNumber == paramRaffleNumber);
        ...
        public List<RaffleEntry> SelectAll(int RaffleId, int ParticipantId)
        {
            ...
            using (var db = new X2MO_InSysEntities())
            {
                ...
                if (ParticipantId >0)
                    raffleEntries = db.RaffleEntries.Where(p=> p.ParticipantId == ParticipantId && p.RaffleId == RaffleId).ToList();
        ...
        public Result DeleteBulk(int paramParticipantId)
        {
            result = new Result();
            using (var db = new X2MO_InSysEntities()){
                db.RaffleEntries.RemoveRange(db.RaffleEntries.Where(p => p.ParticipantId == paramParticipantId));
                db.SaveChanges();
        ...
        public Result DeleteBulk(int paramRaffleId, int paramParticipantId)
        {
            result = new Result();
            using (var db = new X2MO_InSysEntities()){
                db.RaffleEntries.RemoveRange(db.RaffleEntries.Where(p => p.ParticipantId == paramParticipantId && p.RaffleId == paramRaffleId));
                db.SaveChanges();
